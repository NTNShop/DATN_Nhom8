  ...
  useEffect(() => {
    ...
    const fetchCategoriesAndBrands = async () => {
      try {
        ...
        const filteredCategories = categoryResponse.data.data.filter(
          (item) => item.status === 1
        );
        const filteredBrands = brandResponse.data.data.filter(
          (item) => item.status === 1
        );
                        ...
                        {product.variants.map((variant, index) => (
                          ...
                          >
                            <span className="text-dark">
                              Màu: {variant.color} - Giá:{" "}
                              {Number(variant.price).toLocaleString()} VNĐ
                            </span>
                            <button
                              type="button"
