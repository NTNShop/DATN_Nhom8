  ...
  const handleUpdateQuantity = async (cartItemId, newQuantity, currentItem) => {
    ...
    try {
      ...
      setCartItems((prevItems) =>
        prevItems.map((item) => (item.id === cartItemId ? currentItem : item))
      );
  ...
  const handleRemoveItem = async (cartItemId) => {
    try {
      ...
      const newTotalAmount = cartItems
        .filter(
          (item) => selectedItems.includes(item.id) && item.id !== cartItemId
        )
        .reduce(
          (sum, item) => sum + parseFloat(item.unit_price) * item.quantity,
          0
                          ...
                          <div className="quantity">
                            <div className="pro-qty">
                              <button
                                ...
                                onChange={(e) =>
                                  handleUpdateCartItem(item.id, e.target.value)
                                }
